      ...
      <div className="flex-1 overflow-hidden flex flex-col px-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-1 pr-2 -mr-2">
          {conversations.map((conv) => (
            <button
              key={conv.id}
              className={`w-full px-3 py-2.5 rounded-lg flex items-start gap-3 transition-all group ${conv.active
                  ? 'bg-orange-50 border border-orange-100'
                  : 'hover:bg-gray-50 border border-transparent'
                }`}
            >
              <History
                className={`w-4 h-4 mt-0.5 flex-shrink-0 ${conv.active ? 'text-orange-600' : 'text-gray-400 group-hover:text-gray-600'
                  }`}
              />
              <div className="flex-1 text-left overflow-hidden">
                <p
                  className={`text-sm truncate ${conv.active ? 'text-gray-900 font-medium' : 'text-gray-700'
                    }`}
                >
                  {conv.title}
                </p>
